...
pub use local_storage::{AppSettings, LocalStorageManager, PortfolioListItem, StorageInfo};
...
pub fn export_to_json_string(positions: &[crate::models::Position]) -> Result<String, String> {
    serde_json::to_string_pretty(positions).map_err(|e| format!("Serialization error: {}", e))
}

pub fn import_from_json_string(json_data: &str) -> Result<Vec<crate::models::Position>, String> {
    serde_json::from_str(json_data).map_err(|e| format!("Deserialization error: {}", e))
}
