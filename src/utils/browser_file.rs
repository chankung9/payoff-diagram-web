...
use web_sys::{window, File, FileReader, Blob, BlobPropertyBag, Url, HtmlAnchorElement, Document};
use js_sys::{Array, Uint8Array};
use crate::models::{Portfolio, web3_data};
use crate::utils::web3_export_import::{export, import, file_utils};
...
impl BrowserFileManager {
        ...
        let promise = js_sys::Promise::new(&mut |resolve, reject| {
            ...
            let onload_callback = Closure::wrap(Box::new(move |_| {
                if let Ok(result) = reader_clone.result() {
                    ...
                    reject_clone.call1(&JsValue::NULL, &JsValue::from_str("Failed to read file")).unwrap_or_default();
                }
            ...
            let onerror_callback = Closure::wrap(Box::new(move |_| {
                reject_clone2.call1(&JsValue::NULL, &JsValue::from_str("Error reading file")).unwrap_or_default();
            }) as Box<dyn FnMut(JsValue)>);
            ...
            if let Err(_) = file_reader.read_as_text(file) {
                reject.call1(&JsValue::NULL, &JsValue::from_str("Failed to start reading file")).unwrap_or_default();
            }
        });
    ...
    /// Convert legacy positions to new portfolio format
    pub fn convert_positions_to_portfolio(positions: Vec<crate::models::Position>) -> Result<Portfolio, String> {
        use chrono::Utc;
            ...
            .map(|(index, position)| {
                EnhancedPosition {
                    ...
                    metadata: PositionMetadata {
                        ...
                        notes: Some(format!("Imported from legacy format (position #{})", index + 1)),
                        external_id: None,
                    },
                }
            })
...
pub mod drag_drop {
    use super::*;
    use web_sys::{DragEvent, DataTransfer, FileList};
    ...
    pub fn get_files_from_drop_event(event: &DragEvent) -> Result<Vec<File>, String> {
        ...
        let file_list = data_transfer
            .files()
            .ok_or("No files in drop event")?;
