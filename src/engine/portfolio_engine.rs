...
impl PortfolioEngine {
    ...
    pub fn analyze_portfolio(
        ...
        let breakeven_points = PayoffEngine::find_breakeven_points(
            positions, price_start, price_end, step_size
        );
        
        let max_profit = PayoffEngine::calculate_max_profit(
            positions, price_start, price_end, step_size
        );
        
        let max_loss = PayoffEngine::calculate_max_loss(
            positions, price_start, price_end, step_size
        );

        let payoff_curve = PayoffEngine::generate_payoff_curve(
            positions, price_start, price_end, step_size
        );
    ...
    fn calculate_profit_probability(payoff_curve: &[PayoffPoint]) -> Option<f64> {
        ...
        let profitable_points = payoff_curve.iter()
    ...
    fn calculate_expected_value(payoff_curve: &[PayoffPoint]) -> Option<f64> {
        ...
        let total_payoff: f64 = payoff_curve.iter()
    ...
    pub fn has_unlimited_profit(positions: &[Position]) -> bool {
        ...
        positions.iter().any(|pos| match pos {
            ...
            crate::models::Position::Option(option) => {
                matches!(option.option_type, crate::models::OptionType::Call) && option.quantity > 0.0
            },
    ...
    pub fn has_unlimited_loss(positions: &[Position]) -> bool {
        ...
        positions.iter().any(|pos| match pos {
            ...
            crate::models::Position::Option(option) => {
                ...
                (matches!(option.option_type, crate::models::OptionType::Call) && option.quantity < 0.0) ||
                (matches!(option.option_type, crate::models::OptionType::Put) && option.quantity > 0.0)
            },
